...
mod_name = 'xgb_iba_soilmoistureclass_model_xgb-soilmoistureclass_XTRAFF-FIN.json'
...
print(f'Member number {member}')
# Load datasets 

# sfc+sde surface variables: 2t, 2d, rsn, sd, sde, sp, u10, v10, skt, src
sfc_rename = {'d2m': 'td2', 't2m': 't2', 'rsn': 'rsn', 'sd': 'sd', 'sde': 'sde',
              'sp': 'sp', 'u10': 'u10', 'v10': 'v10', 'skt': 'skt','src': 'src'}
sfc_ds = xr.open_dataset(sfc, engine='cfgrib',
    backend_kwargs={'time_dims': ('valid_time', 'verifying_time'), 'indexpath': '',
                    'filter_by_keys': {'typeOfLevel': 'surface'}})[sfc_rename.keys()].rename_vars(sfc_rename)

# sfc depth below land variables: stl1, stl2, swvl1, swvl2, swvl3
stl1_ds = xr.open_dataset(sfc, engine='cfgrib',
    backend_kwargs={'time_dims': ('valid_time', 'verifying_time'), 'indexpath': '',
                    'filter_by_keys': {'shortName': 'stl1'}})
stl2_ds = xr.open_dataset(sfc, engine='cfgrib',
    backend_kwargs={'time_dims': ('valid_time', 'verifying_time'), 'indexpath': '',
                    'filter_by_keys': {'shortName': 'stl2'}})
...
# disaccumulated variables: e, ep ,tp, slhf, sshf, ro, sro, ssro, str, strd, ssr, ssrd, sf
disacc_rename = {'e': 'e', 'pev': 'ep', 'tp': 'tp', 'slhf': 'slhf', 'sshf': 'sshf', 'ro': 'ro', 'sro': 'sro', 'ssro': 'ssro', 'str': 'str', 'strd': 'strd', 'ssr': 'ssr', 'ssrd': 'ssrd', 'sf': 'sf'}
disacc_ds = xr.open_dataset(disacc, engine='cfgrib',
    backend_kwargs={'time_dims': ('valid_time', 'verifying_time'), 'indexpath': '',
                    'filter_by_keys': {'typeOfLevel': 'surface'}}).rename_vars(disacc_rename)
...
# --- Merge all datasets ---
ds = xr.merge([
    sfc_ds, stl1_ds, stl2_ds, swvl1_ds, swvl2_ds, swvl3_ds,
    disacc_ds,
...
#df['closest_hour'] = df.valid_time.dt.hour # not used 
df['doy_sin'] = np.sin(2 * np.pi * df.valid_time.dt.dayofyear / 365.25)
df['doy_cos'] = np.cos(2 * np.pi * df.valid_time.dt.dayofyear / 365.25)
df['accuracy_own'] = 1
