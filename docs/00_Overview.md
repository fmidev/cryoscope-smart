...
- NOTE! Server to service data for Europe **smartmet.xyz** and [https://smartmet.xyz/grid-gui](https://smartmet.xyz/grid-gui) (not to duplicate existing datasets from previous work)
...
From the grid-gui interface user can browse available producers, products (data sets), parameters, and time periods. It is a useful tool for [Timeseries](02_Timeseries.md) requests and other APIs. User can also download grib file from the **Download** link for the snapshot shown in grid-gui view. Let's take a closer look.   
